...
// The Agent sets up and connects all the different components in a service instance.
type Agent struct {
	Config
...
// The Shutdown ensures we shut down the agent once even if shutdown is called multiple times.
// Shutdown is managed gracefully by:
// Leaving the membership so that other instances in the cluster stop sending it events.
