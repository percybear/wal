...
// It needs to hold the configuration so it can create the agent, and
// it needs to hold a reference to the agent so it can shutdown gracefully.
type cli struct {
	cfg cfg
}

// A cfg contains the configuration for the agent.
// It is separates out from the agent.Config the field types that cannot parse
// without error handling: *net.TCPAddr and the *tls.Config.
type cfg struct {
	agent.Config
